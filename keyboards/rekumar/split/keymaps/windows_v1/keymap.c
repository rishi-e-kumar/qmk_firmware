#include QMK_KEYBOARD_H

// Each layer gets a name for readability, which is then used in the keymap matrix below.
// The underscores don't mean anything - you can have a layer called STUFF or any other name.
// Layer names don't all need to be of the same length, obviously, and you can also skip them
// entirely and just use numbers.
#define _QWERTY 0
#define _COLEMAK 1
#define _DVORAK 2
#define _LOWER 3
#define _RAISE 4
#define _MOVE 5
#define _ADJUST 16


enum custom_keycodes {
  QWERTY = SAFE_RANGE,
  COLEMAK,
  DVORAK
};

#define LOWER  MO(_LOWER)
#define RAISE  MO(_RAISE)
#define ADJUST MO(_ADJUST)
#define MOVE   MO(_MOVE)

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
/* Qwerty */
[_QWERTY] = LAYOUT(
    KC_ESC,   KC_1,   KC_2,   KC_3,   KC_4,   KC_5,           KC_6,   KC_7,   KC_8,   KC_9,   KC_0,   KC_BSPC, \
    KC_TAB,   KC_Q,   KC_W,   KC_E,   KC_R,   KC_T,           KC_Y,   KC_U,   KC_I,   KC_O,   KC_P,   KC_COMM, \
    KC_LSFT,  KC_A,   KC_S,   KC_D,   KC_F,   KC_G,           KC_H,   KC_J,   KC_K,   KC_L,   KC_SCLN,KC_DOT, \
    KC_LCTL,  KC_Z,   KC_X,   KC_C,   KC_V,   KC_B,           KC_N,   KC_M,   KC_COMM,KC_DOT, KC_SLSH,KC_RSFT, \
        KC_LALT,KC_LGUI, LT(_MOVE, KC_SPC), LOWER,            RAISE,  LSFT_T(KC_BSPC),LCTL_T(KC_ENT)             \
),

/* Move */
[_MOVE] = LAYOUT( \
   KC_ESC,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,       XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX, \
   KC_TAB,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,       KC_HOME,KC_PGUP,KC_PGDN,KC_END ,XXXXXXX,XXXXXXX ,\
   KC_LSFT,XXXXXXX,KC_LALT,KC_LSFT,KC_LCTL,KC_LGUI,       KC_LEFT,KC_UP  ,KC_DOWN,KC_RGHT,XXXXXXX,XXXXXXX, \
   KC_LCTL,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,       C(G(KC_LEFT)),XXXXXXX,XXXXXXX,C(G(KC_RGHT)),XXXXXXX, KC_RSFT, \
        KC_LALT,KC_LGUI, LT(_MOVE, KC_SPC), LOWER,       RAISE,  LSFT_T(KC_DEL), LCTL_T(KC_ENT)                          \
),


/* Lower */
[_LOWER] = LAYOUT( \
   KC_ESC,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,       XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,  XXXXXXX, \
   KC_TAB,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,       KC_COLN,KC_AMPR,KC_TILD,KC_EXLM,XXXXXXX,KC_COMM, \
   KC_LSFT,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,       KC_ASTR,KC_PLUS,KC_EQL, KC_MINS,KC_SLSH,KC_BSLS, \
   KC_LCTL,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,XXXXXXX,       KC_CIRC,KC_LABK,KC_PIPE,KC_RABK,KC_PERC,KC_RSFT, \
        KC_LALT,KC_LGUI, LT(_MOVE, KC_SPC), LOWER,       RAISE,  LSFT_T(KC_DEL), LCTL_T(KC_ENT)                          \
),

/* Raise */
[_RAISE] = LAYOUT( \
  KC_ESC,  KC_F1  ,KC_F2   ,KC_F3  ,  KC_F4 ,  KC_F5,         KC_F6,  KC_F7,  KC_F8,  KC_F9  ,KC_F10, KC_F11 , \
  KC_TAB , XXXXXXX, KC_DQUO, KC_LCBR, KC_RCBR, KC_COLN,       XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,  KC_F12, \
  KC_LSFT , KC_COLN, KC_QUOT, KC_LPRN, KC_RPRN, KC_UNDS,       XXXXXXX, KC_VOLD, KC_MUTE, KC_VOLU,XXXXXXX,  XXXXXXX, \
  KC_LCTL , XXXXXXX, XXXXXXX, KC_LBRC, KC_RBRC, KC_HASH,       XXXXXXX, KC_MPRV, KC_MPLY, KC_MNXT,XXXXXXX, KC_RSFT, \
            KC_LALT, KC_LGUI, LT(_MOVE, KC_SPC), LOWER,        RAISE,  LSFT_T(KC_DEL), LCTL_T(KC_ENT)                          \

),

/* Adjust (Lower + Raise)
 * ,-----------------------------------------------------------------------------------.
 * |      | Reset|      |      |      |      |      |      |      |      |      |  Del |
 * |------+------+------+------+------+-------------+------+------+------+------+------|
 * |      |      |      |Aud on|Audoff|AGnorm|AGswap|Qwerty|Colemk|Dvorak|      |      |
 * |------+------+------+------+------+------|------+------+------+------+------+------|
 * |      |      |      |      |      |      |      |      |      |      |      |      |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |             |      |      |      |      |      |
 * `-----------------------------------------------------------------------------------'
 */
[_ADJUST] =  LAYOUT( \
  XXXXXXX, RESET,   XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,   XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, KC_DEL, \
  XXXXXXX, XXXXXXX, MAGIC_UNSWAP_CTL_GUI, XXXXXXX, XXXXXXX, XXXXXXX,   XXXXXXX, QWERTY , XXXXXXX, XXXXXXX,  XXXXXXX, XXXXXXX, \
  XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,   XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, \
  XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,   XXXXXXX, MAGIC_SWAP_CTL_GUI, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, \
                   LALT_T(KC_LGUI),KC_LSFT, KC_SPC, LOWER,  RAISE,  LCTL_T(KC_DEL), LSFT(KC_ENT)                          \

)


};

layer_state_t layer_state_set_user(layer_state_t state) {
  return update_tri_layer_state(state, _LOWER, _RAISE, _ADJUST);
}

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    case QWERTY:
      if (record->event.pressed) {
        set_single_persistent_default_layer(_QWERTY);
      }
      return false;
    case COLEMAK:
      if (record->event.pressed) {
        set_single_persistent_default_layer(_COLEMAK);
      }
      return false;
    case DVORAK:
      if (record->event.pressed) {
        set_single_persistent_default_layer(_DVORAK);
      }
      return false;
  }
  return true;
}


